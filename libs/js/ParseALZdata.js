        ...
        if(rawFile.readyState === 4)
        {
            if(rawFile.status === 200 || rawFile.status == 0)
            {
                ...
                do {
                    ...
                    var G247 = "<img src='libs/icons/ALZ.png' style='width:40px;height:40px;'>'" ;

                    if (other[i] == ""){
                        extraText = "<a href='" + Link[i] + "' target='_blank'>" + G247 +"</a><br>"+FNresult[i]+"<br>"+Contact[i]+"<br>"+What[i]+"<br>"+Address[i]+"<br>"+phone[i]+"<br>";
                    } else {
                        extraText = "<a href='" + Link[i] + "' target='_blank'>" + G247 +"</a><br>"+FNresult[i]+"<br>"+Contact[i]+"<br>"+What[i]+"<br>"+Address[i]+"<br>"+phone[i]+"<br>"+other[i]+"<br>";
                    };

                    if(varX[i] != undefined){getALZLatLon(varX[i], varY[i], FNresult[i], other[i])}
...
function customTip(ShortTip) {
  ...
  function getALZLatLon(thisLat, thisLng, ALZname, other){
    //console.log(strSearch + "mark");
        var ALZDatashow =
        {
          ...
          'className' : 'ALZ-popup'
            ...
            if (other == "24/7"){
                var ALZMarker = L.marker([thisLat, thisLng], {icon: ALZIcon}).addTo(ALZMarkers)
            } else {
                var ALZMarker = L.marker([thisLat, thisLng], {icon: ALZ2Icon}).addTo(ALZMarkers)
            };                  
                //var ALZMarker = L.marker([thisLat, thisLng], {icon: ALZIcon}).addTo(ALZMarkers)


//            var OPmarker = L.marker([thisLat, thisLng], {icon: OPIcon}).addTo(OpTBMMarkers)
        var markerPopup = ALZMarker.bindPopup(extraText, ALZDatashow)
        ...
        markerPopup = ALZMarker.bindTooltip(varTooltip);
        markerPopup = ALZMarker.on('mouseover', showHideTooltip);
