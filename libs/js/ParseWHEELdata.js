        ...
        if(rawFile.readyState === 4)
        {
            if(rawFile.status === 200 || rawFile.status == 0)
            {
                ...
                do {
                    ...
                    if (other[i] == "COMMUNITY CENTRE OUTREACH SERVICE"){
                        var G247 = "<img src='libs/icons/wheel.png' style='width:40px;height:40px;'>'" ;
                    } else {
                        var G247 = "<img src='libs/icons/wheel.png' style='width:40px;height:40px;'>'" ;
                    };
                    ...
                    if(varX[i] != undefined){getWHEELLatLon(varX[i], varY[i], FNresult[i], other[i])}
...
function customTip(ShortTip) {
  ...
  function getWHEELLatLon(thisLat, thisLng, WHEELname, other){
    //console.log(strSearch + "mark");
        var WHEELDatashow =
        {
          ...
          'className' : 'WHEEL-popup'
            ...
            if (other == "24/7"){
                var WHEELMarker = L.marker([thisLat, thisLng], {icon: WHEELIcon}).addTo(WHEELMarkers)
            } else {
                var WHEELMarker = L.marker([thisLat, thisLng], {icon: WHEEL2Icon}).addTo(WHEELMarkers)
            };                  
                //var WHEELMarker = L.marker([thisLat, thisLng], {icon: WHEELIcon}).addTo(WHEELMarkers)


//            var OPmarker = L.marker([thisLat, thisLng], {icon: OPIcon}).addTo(OpTBMMarkers)
        var markerPopup = WHEELMarker.bindPopup(extraText, WHEELDatashow)

        varTooltip = WHEELname;
        //if (Pets == "Yes"){
        ...
        markerPopup = WHEELMarker.bindTooltip(varTooltip);
        markerPopup = WHEELMarker.on('mouseover', showHideTooltip);
